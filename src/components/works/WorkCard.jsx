...
export default function WorkCard({stack,img,title}){
    const {mode} = useContext(ThemeContext)
    return (
     <Link href="#" className={`relative group flex flex-col items-start w-[24%] pb-5 rounded-2xl shadow-custom-shadow ${mode==="light" ? "" : "bg-[#2c072c]"} overflow-hidden wave`}>
        ...
        <div className="block mt-3 ml-2 font-bold gradient group-hover:text-[green] transition-all duration-500 ease-in-out">
            <h1>{title}</h1>
        </div>
     </Link>
    )
